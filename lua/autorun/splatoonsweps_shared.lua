...
function ss:SetPrimary(weapon, info)
	...
	weapon.Primary = p
	if isfunction(weapon.CustomPrimary) then return weapon:CustomPrimary(p, info) end
end
...
function ss:SetSecondary(weapon, info)
	...
	weapon.Secondary = s
	if isfunction(weapon.CustomSecondary) then return weapon:CustomSecondary(s, info) end
end
