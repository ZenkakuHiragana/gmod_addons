...
if CLIENT then
	function SWEP:PreViewModelDrawn(model, bone_ent, ang, pos, v, matrix)
		----------------------------------------
		if self.Owner:IsPlayer() and self:Ammo1() > 0 then
			render.DrawLine(model:GetPos() + Vector(0, 0, 1),
				model:GetPos() + Vector(0, 0, 1) + self.Owner:GetForward() * 10000,
				self.ProjColor, true)
		end
...
function SWEP:IsInkling(isinkling)
	if self.Owner:IsPlayer() then
		...
		if self.charge > 0 and not self.Owner:KeyDown(IN_ATTACK) then
			self:FireInk()
		end
	else
...
--TODO: fix minimum charge shot distance
function SWEP:paint()
