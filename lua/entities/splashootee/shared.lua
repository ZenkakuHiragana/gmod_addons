...
--		self.AngleDif = Angle(-90, 0, 0)
--		self:SetAngles(self:GetAngles() + self.AngleDif)
--		self:setCollision(Vector(medium, medium, medium))
--	elseif self.size == 2 then
--		self:SetModel("models/hunter/blocks/cube05x05x025.mdl")
--		self.AngleDif = Angle(90, 0, 0)
--		self:SetAngles(self:GetAngles() + self.AngleDif)
--		self:setCollision(Vector(small, small, small))
--	elseif self.size == 3 then
--		self:SetModel("models/hunter/blocks/cube025x025x025.mdl")
--		self.AngleDif = Angle(90, 0, 0)
--		self:SetAngles(self:GetAngles() + self.AngleDif)
--		self:setCollision(Vector(micro, micro, micro))
	end
end
...
function ENT:Touch(t)
	...
	if t:IsNPC() and t.GetActiveWeapon ~= nil then
		...
		--	tr:SetPos(self:GetPos())
		--	tr:Spawn()
		--	SafeRemoveEntityDelayed(tr, 3)
		--	t:SetEnemy(tr)
		--	t:UpdateEnemyMemory(tr, tr:GetPos())
		--	t:SetSchedule(SCHED_RANGE_ATTACK2)
		end
	end
...
--			--self:DisableMatrix("RenderMultiply")
--			local a, h = self:GetNWVector("normal", Vector(0, 0, 0)), self:GetNWVector("hull", Vector(big, big, big)):Le--ngth() * 3
