
function ENT:GetAct(t)
	if IsValid(self.Weapon) then
		self.ActivityTranslateAI = {}
		if self.HoldType == "none" then
			self.ActivityTranslateAI [ ACT_IDLE ] 						= ACT_IDLE
			self.ActivityTranslateAI [ ACT_IDLE_ANGRY ] 				= ACT_IDLE_ANGRY
			self.ActivityTranslateAI [ ACT_IDLE_RELAXED ] 				= ACT_IDLE_RELAXED
			self.ActivityTranslateAI [ ACT_IDLE_STIMULATED ] 			= ACT_IDLE_STIMULATED
			self.ActivityTranslateAI [ ACT_IDLE_AGITATED ] 				= ACT_IDLE_AGITATED
			self.ActivityTranslateAI [ ACT_IDLE_ANGRY ] 				= ACT_IDLE_ANGRY
			self.ActivityTranslateAI [ ACT_HL2MP_IDLE ]					= ACT_HL2MP_IDLE
			self.ActivityTranslateAI [ ACT_HL2MP_IDLE_ANGRY ]			= ACT_HL2MP_IDLE_ANGRY
			self.ActivityTranslateAI [ ACT_HL2MP_IDLE_CROUCH ]			= ACT_HL2MP_IDLE_CROUCH
			self.ActivityTranslateAI [ ACT_HL2MP_IDLE_CROUCH_ANGRY ]	= ACT_HL2MP_IDLE_CROUCH_ANGRY
			self.ActivityTranslateAI [ ACT_WALK_AIM ] 					= ACT_WALK_AIM
			self.ActivityTranslateAI [ ACT_RUN_AIM ] 					= ACT_RUN_AIM
			self.ActivityTranslateAI [ ACT_RUN_CROUCH ] 				= ACT_RUN_CROUCH
			self.ActivityTranslateAI [ ACT_HL2MP_RUN ]					= ACT_HL2MP_RUN
			self.ActivityTranslateAI [ ACT_HL2MP_RUN_ANGRY ]			= ACT_HL2MP_RUN_ANGRY
			self.ActivityTranslateAI [ ACT_HL2MP_JUMP ]					= ACT_HL2MP_JUMP
			self.ActivityTranslateAI [ ACT_HL2MP_JUMP_ANGRY ]			= ACT_HL2MP_JUMP_ANGRY
			
			self.ActivityTranslateAI [ ACT_RANGE_ATTACK1 ] 				= ACT_IDLE
			self.ActivityTranslateAI [ ACT_RANGE_ATTACK1_LOW ]			= ACT_RANGE_ATTACK1_LOW
			self.ActivityTranslateAI [ ACT_GESTURE_RANGE_ATTACK1 ] 		= ACT_GESTURE_RANGE_ATTACK1
			self.ActivityTranslateAI [ ACT_GESTURE_RANGE_ATTACK2 ] 		= ACT_GESTURE_RANGE_ATTACK2
			self.ActivityTranslateAI [ ACT_HL2MP_GESTURE_RANGE_ATTACK ]	= ACT_HL2MP_GESTURE_RANGE_ATTACK
			self.ActivityTranslateAI [ ACT_GESTURE_RANGE_ATTACK1_LOW ]	= ACT_GESTURE_RANGE_ATTACK1_LOW
			self.ActivityTranslateAI [ ACT_GESTURE_RANGE_ATTACK2_LOW ]	= ACT_GESTURE_RANGE_ATTACK2_LOW
			self.ActivityTranslateAI [ ACT_RELOAD ] 					= ACT_RELOAD
			self.ActivityTranslateAI [ ACT_RELOAD_LOW ] 				= ACT_RELOAD_LOW
			self.ActivityTranslateAI [ ACT_GESTURE_RELOAD ] 			= ACT_GESTURE_RELOAD
			self.ActivityTranslateAI [ ACT_HL2MP_GESTURE_RELOAD ]		= ACT_HL2MP_GESTURE_RELOAD
			self.ActivityTranslateAI [ ACT_HL2MP_GESTURE_RELOAD_ANGRY ]	= ACT_HL2MP_GESTURE_RELOAD_ANGRY
			self.ActivityTranslateAI [ ACT_COVER_LOW ] 					= ACT_COVER_LOW
			self.ActivityTranslateAI [ ACT_RANGE_AIM_LOW ] 				= ACT_RANGE_AIM_LOW
			
		elseif self.HoldType == "smg" then
			self.ActivityTranslateAI [ ACT_IDLE ] 						= ACT_IDLE_SMG1
			self.ActivityTranslateAI [ ACT_IDLE_ANGRY ] 				= ACT_IDLE_ANGRY_SMG1
			self.ActivityTranslateAI [ ACT_IDLE_RELAXED ] 				= ACT_IDLE_SMG1_RELAXED
			self.ActivityTranslateAI [ ACT_IDLE_STIMULATED ] 			= ACT_IDLE_SMG1_STIMULATED
			self.ActivityTranslateAI [ ACT_IDLE_AGITATED ] 				= ACT_IDLE_SMG1_RELAXED
			self.ActivityTranslateAI [ ACT_IDLE_ANGRY ] 				= ACT_IDLE_ANGRY_SMG1
			self.ActivityTranslateAI [ ACT_HL2MP_IDLE ]					= ACT_HL2MP_IDLE_SMG1
			self.ActivityTranslateAI [ ACT_HL2MP_IDLE_ANGRY ]			= ACT_HL2MP_IDLE_SMG1
			self.ActivityTranslateAI [ ACT_HL2MP_IDLE_CROUCH ]			= ACT_HL2MP_IDLE_CROUCH_SMG1
			self.ActivityTranslateAI [ ACT_HL2MP_IDLE_CROUCH_ANGRY ]	= ACT_HL2MP_IDLE_CROUCH_SMG1
			self.ActivityTranslateAI [ ACT_WALK ] 						= ACT_WALK_RIFLE_STIMULATED
			self.ActivityTranslateAI [ ACT_WALK_AIM ] 					= ACT_WALK_AIM_RIFLE_STIMULATED	
			self.ActivityTranslateAI [ ACT_RUN ] 						= ACT_RUN_RIFLE_STIMULATED
			self.ActivityTranslateAI [ ACT_RUN_AIM ] 					= ACT_RUN_AIM_RIFLE_STIMULATED
			self.ActivityTranslateAI [ ACT_RUN_CROUCH ] 				= ACT_RUN_CROUCH_RIFLE
			self.ActivityTranslateAI [ ACT_HL2MP_RUN ]					= ACT_HL2MP_RUN_SMG1
			self.ActivityTranslateAI [ ACT_HL2MP_RUN_ANGRY ]			= ACT_HL2MP_RUN_SMG1
			self.ActivityTranslateAI [ ACT_HL2MP_JUMP ]					= ACT_HL2MP_JUMP_SMG1
			self.ActivityTranslateAI [ ACT_HL2MP_JUMP_ANGRY ]			= ACT_HL2MP_JUMP_SMG1
			
			self.ActivityTranslateAI [ ACT_RANGE_ATTACK1 ] 				= ACT_RANGE_ATTACK_SMG1
			self.ActivityTranslateAI [ ACT_RANGE_ATTACK1_LOW ]			= ACT_RANGE_ATTACK_SMG1_LOW
			self.ActivityTranslateAI [ ACT_GESTURE_RANGE_ATTACK1 ] 		= ACT_GESTURE_RANGE_SMG1
			self.ActivityTranslateAI [ ACT_GESTURE_RANGE_ATTACK2 ] 		= ACT_GESTURE_RANGE_SMG2
			self.ActivityTranslateAI [ ACT_HL2MP_GESTURE_RANGE_ATTACK ]	= ACT_HL2MP_GESTURE_RANGE_ATTACK_SMG1
			self.ActivityTranslateAI [ ACT_GESTURE_RANGE_ATTACK1_LOW ]	= ACT_GESTURE_RANGE_ATTACK_SMG1_LOW
			self.ActivityTranslateAI [ ACT_RELOAD ] 					= ACT_RELOAD_SMG1
			self.ActivityTranslateAI [ ACT_RELOAD_LOW ] 				= ACT_RELOAD_SMG1_LOW
			self.ActivityTranslateAI [ ACT_GESTURE_RELOAD ] 			= ACT_GESTURE_RELOAD_SMG1
			self.ActivityTranslateAI [ ACT_HL2MP_GESTURE_RELOAD ]		= ACT_HL2MP_GESTURE_RELOAD_SMG1
			self.ActivityTranslateAI [ ACT_HL2MP_GESTURE_RELOAD_ANGRY ]	= ACT_HL2MP_GESTURE_RELOAD_SMG1
			self.ActivityTranslateAI [ ACT_COVER_LOW ] 					= ACT_COVER_SMG1_LOW
			self.ActivityTranslateAI [ ACT_RANGE_AIM_LOW ] 				= ACT_RANGE_AIM_SMG1_LOW
			
		elseif self.HoldType == "ar2" then
			self.ActivityTranslateAI [ ACT_RANGE_ATTACK1 ] 				= ACT_RANGE_ATTACK_AR2
			self.ActivityTranslateAI [ ACT_RELOAD ] 					= ACT_RELOAD_SMG1
			self.ActivityTranslateAI [ ACT_IDLE ] 						= ACT_IDLE_RIFLE
			self.ActivityTranslateAI [ ACT_IDLE_ANGRY ] 				= ACT_IDLE_ANGRY_SMG1
			self.ActivityTranslateAI [ ACT_WALK ] 						= ACT_WALK_RIFLE

			self.ActivityTranslateAI [ ACT_IDLE_RELAXED ] 				= ACT_IDLE_SMG1_RELAXED
			self.ActivityTranslateAI [ ACT_IDLE_STIMULATED ] 			= ACT_IDLE_SMG1_STIMULATED
			self.ActivityTranslateAI [ ACT_IDLE_AGITATED ] 				= ACT_IDLE_ANGRY_SMG1

			self.ActivityTranslateAI [ ACT_WALK_RELAXED ] 				= ACT_WALK_RIFLE_RELAXED
			self.ActivityTranslateAI [ ACT_WALK_STIMULATED ] 			= ACT_WALK_RIFLE_STIMULATED
			self.ActivityTranslateAI [ ACT_WALK_AGITATED ] 				= ACT_WALK_AIM_RIFLE

			self.ActivityTranslateAI [ ACT_RUN_RELAXED ] 				= ACT_RUN_RIFLE_RELAXED
			self.ActivityTranslateAI [ ACT_RUN_STIMULATED ] 			= ACT_RUN_RIFLE_STIMULATED
			self.ActivityTranslateAI [ ACT_RUN_AGITATED ] 				= ACT_RUN_AIM_RIFLE

			self.ActivityTranslateAI [ ACT_IDLE_AIM_RELAXED ] 			= ACT_IDLE_SMG1_RELAXED
			self.ActivityTranslateAI [ ACT_IDLE_AIM_STIMULATED ] 		= ACT_IDLE_AIM_RIFLE_STIMULATED
			self.ActivityTranslateAI [ ACT_IDLE_AIM_AGITATED ] 			= ACT_IDLE_ANGRY_SMG1

			self.ActivityTranslateAI [ ACT_WALK_AIM_RELAXED ] 			= ACT_WALK_RIFLE_RELAXED
			self.ActivityTranslateAI [ ACT_WALK_AIM_STIMULATED ] 		= ACT_WALK_AIM_RIFLE_STIMULATED
			self.ActivityTranslateAI [ ACT_WALK_AIM_AGITATED ] 			= ACT_WALK_AIM_RIFLE

			self.ActivityTranslateAI [ ACT_RUN_AIM_RELAXED ] 			= ACT_RUN_RIFLE_RELAXED
			self.ActivityTranslateAI [ ACT_RUN_AIM_STIMULATED ] 		= ACT_RUN_AIM_RIFLE_STIMULATED
			self.ActivityTranslateAI [ ACT_RUN_AIM_AGITATED ] 			= ACT_RUN_AIM_RIFLE

			self.ActivityTranslateAI [ ACT_WALK_AIM ] 					= ACT_WALK_AIM_RIFLE
			self.ActivityTranslateAI [ ACT_WALK_CROUCH ] 				= ACT_WALK_CROUCH_RIFLE
			self.ActivityTranslateAI [ ACT_WALK_CROUCH_AIM ] 			= ACT_WALK_CROUCH_AIM_RIFLE
			self.ActivityTranslateAI [ ACT_RUN ] 						= ACT_RUN_RIFLE
			self.ActivityTranslateAI [ ACT_RUN_AIM ] 					= ACT_RUN_AIM_RIFLE
			self.ActivityTranslateAI [ ACT_RUN_CROUCH ] 				= ACT_RUN_CROUCH_RIFLE
			self.ActivityTranslateAI [ ACT_RUN_CROUCH_AIM ] 			= ACT_RUN_CROUCH_AIM_RIFLE
			self.ActivityTranslateAI [ ACT_GESTURE_RANGE_ATTACK1 ] 		= ACT_GESTURE_RANGE_ATTACK_AR2
			self.ActivityTranslateAI [ ACT_COVER_LOW ] 					= ACT_COVER_SMG1_LOW
			self.ActivityTranslateAI [ ACT_RANGE_AIM_LOW ] 				= ACT_RANGE_AIM_AR2_LOW
			self.ActivityTranslateAI [ ACT_RANGE_ATTACK1_LOW ] 			= ACT_RANGE_ATTACK_SMG1_LOW
			self.ActivityTranslateAI [ ACT_RELOAD_LOW ] 				= ACT_RELOAD_SMG1_LOW
			self.ActivityTranslateAI [ ACT_GESTURE_RELOAD ] 			= ACT_GESTURE_RELOAD_SMG1
			
		elseif self.HoldType == "shotgun" then
			self.ActivityTranslateAI [ ACT_RANGE_ATTACK1 ] 				= ACT_RANGE_ATTACK_SHOTGUN
			self.ActivityTranslateAI [ ACT_RANGE_ATTACK1_LOW ] 			= ACT_RANGE_ATTACK_SHOTGUN_LOW
			self.ActivityTranslateAI [ ACT_RELOAD ] 					= ACT_RELOAD_SHOTGUN
			self.ActivityTranslateAI [ ACT_RELOAD_LOW ] 				= ACT_RELOAD_SHOTGUN_LOW
			self.ActivityTranslateAI [ ACT_GESTURE_RANGE_ATTACK1 ] 		= ACT_GESTURE_RANGE_ATTACK_SHOTGUN
			self.ActivityTranslateAI [ ACT_GESTURE_RELOAD ] 			= ACT_GESTURE_RELOAD_SHOTGUN
			
			self.ActivityTranslateAI [ ACT_IDLE_ANGRY ] 				= ACT_IDLE_ANGRY_SHOTGUN
			self.ActivityTranslateAI [ ACT_IDLE_AGITATED ] 				= ACT_IDLE_SHOTGUN_AGITATED
			self.ActivityTranslateAI [ ACT_IDLE_RELAXED ] 				= ACT_IDLE_SHOTGUN_RELAXED
			self.ActivityTranslateAI [ ACT_IDLE_STIMULATED ] 			= ACT_IDLE_SHOTGUN_STIMULATED
			self.ActivityTranslateAI [ ACT_WALK_AIM ] 					= ACT_WALK_AIM_SHOTGUN
			self.ActivityTranslateAI [ ACT_RUN_AIM ] 					= ACT_RUN_AIM_SHOTGUN
			
		elseif self.HoldType == "pistol" then
			self.ActivityTranslateAI [ ACT_IDLE ] 						= ACT_IDLE_PISTOL
			self.ActivityTranslateAI [ ACT_HL2MP_IDLE ] 				= ACT_HL2MP_IDLE_PISTOL
			self.ActivityTranslateAI [ ACT_IDLE_ANGRY ] 				= ACT_IDLE_ANGRY_PISTOL
			self.ActivityTranslateAI [ ACT_RANGE_ATTACK1 ] 				= ACT_RANGE_ATTACK_PISTOL
			self.ActivityTranslateAI [ ACT_RELOAD ] 					= ACT_RELOAD_PISTOL
			self.ActivityTranslateAI [ ACT_WALK_AIM ] 					= ACT_WALK_AIM_PISTOL
			self.ActivityTranslateAI [ ACT_RUN_AIM ] 					= ACT_RUN_AIM_PISTOL
			self.ActivityTranslateAI [ ACT_HL2MP_RUN ] 					= ACT_HL2MP_RUN_PISTOL
			self.ActivityTranslateAI [ ACT_GESTURE_RANGE_ATTACK1 ] 		= ACT_GESTURE_RANGE_ATTACK_PISTOL
			self.ActivityTranslateAI [ ACT_RELOAD_LOW ] 				= ACT_RELOAD_PISTOL_LOW
			self.ActivityTranslateAI [ ACT_RANGE_ATTACK1_LOW ] 			= ACT_RANGE_ATTACK_PISTOL_LOW
			self.ActivityTranslateAI [ ACT_COVER_LOW ] 					= ACT_COVER_PISTOL_LOW
			self.ActivityTranslateAI [ ACT_RANGE_AIM_LOW ] 				= ACT_RANGE_AIM_PISTOL_LOW
			self.ActivityTranslateAI [ ACT_GESTURE_RELOAD ] 			= ACT_GESTURE_RELOAD_PISTOL
			
		elseif self.HoldType == "rpg" then
			self.ActivityTranslateAI [ ACT_IDLE ] 						= ACT_IDLE_RPG
			self.ActivityTranslateAI [ ACT_IDLE_RELAXED ] 				= ACT_IDLE_RPG_RELAXED
			self.ActivityTranslateAI [ ACT_IDLE_ANGRY ] 				= ACT_IDLE_ANGRY_RPG
			self.ActivityTranslateAI [ ACT_WALK ] 						= ACT_WALK_RPG
			self.ActivityTranslateAI [ ACT_WALK_RELAXED ] 				= ACT_WALK_RPG_RELAXED
			self.ActivityTranslateAI [ ACT_WALK_CROUCH ] 				= ACT_WALK_CROUCH_RPG
			self.ActivityTranslateAI [ ACT_RUN_CROUCH ] 				= ACT_RUN_CROUCH_RPG
			self.ActivityTranslateAI [ ACT_RUN ] 						= ACT_RUN_RPG
			self.ActivityTranslateAI [ ACT_RUN_RELAXED ] 				= ACT_RUN_RPG_RELAXED
			self.ActivityTranslateAI [ ACT_COVER_LOW ] 					= ACT_COVER_LOW_RPG
			self.ActivityTranslateAI [ ACT_RANGE_ATTACK1 ] 				= ACT_RANGE_ATTACK_RPG
			
		elseif self.HoldType == "melee" then
			self.ActivityTranslateAI [ ACT_IDLE ] 						= ACT_IDLE
			self.ActivityTranslateAI [ ACT_IDLE_RELAXED ] 				= ACT_IDLE
			self.ActivityTranslateAI [ ACT_IDLE_ANGRY ] 				= ACT_IDLE_ANGRY_MELEE
			self.ActivityTranslateAI [ ACT_WALK ] 						= ACT_WALK
			self.ActivityTranslateAI [ ACT_WALK_RELAXED ] 				= ACT_WALK_RELAXED
			self.ActivityTranslateAI [ ACT_WALK_CROUCH ] 				= ACT_WALK_CROUCH
			self.ActivityTranslateAI [ ACT_RUN_CROUCH ] 				= ACT_RUN_CROUCH
			self.ActivityTranslateAI [ ACT_RUN ] 						= ACT_RUN
			self.ActivityTranslateAI [ ACT_RUN_RELAXED ] 				= ACT_RUN_RELAXED
			self.ActivityTranslateAI [ ACT_COVER_LOW ] 					= ACT_COVER_LOW
			self.ActivityTranslateAI [ ACT_RANGE_ATTACK1 ] 				= ACT_MELEE_ATTACK_SWING
			self.ActivityTranslateAI [ ACT_RANGE_ATTACK2 ] 				= ACT_MELEE_ATTACK2
			self.ActivityTranslateAI [ ACT_GESTURE_RANGE_ATTACK1 ] 		= ACT_MELEE_ATTACK_SWING_GESTURE
			self.ActivityTranslateAI [ ACT_GESTURE_RANGE_ATTACK2 ] 		= ACT_GESTURE_MELEE_ATTACK2
			
		end
		
		return self.ActivityTranslateAI[t] or t
	else
		return t
	end
end
