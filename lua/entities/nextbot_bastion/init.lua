...
local OnSpawnEnemy = CurTime()
local OnSpawnAlly = CurTime()
function ENT:Initialize()
	...
	if self.Relationship[CLASS_PLAYER] == D_HT and CurTime() > OnSpawnEnemy then
	--	self:EmitSound("Nextbot_Tracer.OnSpawn")
		OnSpawnEnemy = CurTime() + math.Rand(15, 30)
	elseif self.Relationship[CLASS_PLAYER] ~= D_HT and CurTime() > OnSpawnAlly then
	--	self:EmitSound("Nextbot_Tracer.OnSpawnAlly")
		OnSpawnAlly = CurTime() + math.Rand(15, 30)
	end
end
----------------------------------------------}
...
function ENT:RunBehaviour()
	...
	self:StartActivity(self.Act.Walk)
	while true do
		self:MoveToPos(self:GetPos() + self:GetForward() * 1000)
		coroutine.yield()
		coroutine.wait(1)
	end
end